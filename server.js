...
const chatHistory = [
  {
    ...
    parts: [
      {
        text: `you're a chatbot here to assist me. Say "use /open" when I ask you to open something and if i use"/open", then say nothing`,
      },
    ],
  },
  {
    ...
    parts: [
      {
        text: `Great to meet you. Sure, i will direct you to use "use /open" to open webpages when you need. How can I help you today?`,
      },
    ],
  },
];
...
app.post("/googleSearch", async (req, res) => {
  ...
  const googleSearchApiUrl = `https://www.googleapis.com/customsearch/v1?q=${encodeURIComponent(
    req.body.formattedQuery
  )}&key=${googleApiKeyFallback}&cx=${cx}`;
...
app.post("/imageSearch", async (req, res) => {
  ...
  const googleSearchApiUrl = `https://www.googleapis.com/customsearch/v1?q=${encodeURIComponent(
    query
  )}&key=${googleApiKeyFallback}&cx=${customSearchEngineId}&searchType=image`;
  ...
  try {
    ...
    console.error(
      "Error fetching image search results from Google:",
      error.message
    );
    res.status(500).json({
      error: "An error occurred while fetching data. Please try again later.",
